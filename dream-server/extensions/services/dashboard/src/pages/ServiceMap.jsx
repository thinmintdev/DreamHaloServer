...
function computeLayout(nodes) {
  ...
  for (const layer of LAYER_ORDER) {
    ...
    if (count === 0) continue
    const rw = count * NODE_W + (count - 1) * NODE_GAP_X
  ...
  for (const layer of LAYER_ORDER) {
    ...
    for (let i = 0; i < row.length; i++) {
      positions[row[i].id] = { x: startX + i * (NODE_W + NODE_GAP_X), y: currentY }
    }
    currentY += LAYER_GAP_Y
  }

  return { positions, svgWidth, svgHeight: currentY + 30 }
}
...
// Compute per-edge port attachment points
// All edges: exit BOTTOM of source → arrive TOP of target
function computePorts(edges, positions) {
  const outByNode = {}  // nodeId → [edge]
  const inByNode  = {}  // nodeId → [edge]

  for (const edge of edges) {
    if (!outByNode[edge.source]) outByNode[edge.source] = []
    if (!inByNode[edge.target])  inByNode[edge.target]  = []
    outByNode[edge.source].push(edge)
    inByNode[edge.target].push(edge)
  }

  // Stable ordering within each node's port list
  for (const arr of Object.values(outByNode)) arr.sort((a, b) => a.target.localeCompare(b.target))
  for (const arr of Object.values(inByNode))  arr.sort((a, b) => a.source.localeCompare(b.source))

  const ports = {}

  for (const [nodeId, outs] of Object.entries(outByNode)) {
    const pos = positions[nodeId]
    if (!pos) continue
    outs.forEach((edge, i) => {
      const k = `${edge.source}-${edge.target}`
      if (!ports[k]) ports[k] = {}
      ports[k].sx = spreadX(pos, outs.length, i)
      ports[k].sy = pos.y + NODE_H   // exit bottom
    })
  }

  for (const [nodeId, ins] of Object.entries(inByNode)) {
    const pos = positions[nodeId]
    if (!pos) continue
    ins.forEach((edge, i) => {
      const k = `${edge.source}-${edge.target}`
      if (!ports[k]) ports[k] = {}
      ports[k].tx = spreadX(pos, ins.length, i)
      ports[k].ty = pos.y   // arrive top
    })
  }
...
// Orthogonal elbow path from (sx, sy) bottom-of-source to (tx, ty) top-of-target.
// Same-layer edges loop below the row; cross-layer edges use a midpoint elbow.
function elbowPath(sx, sy, tx, ty) {
  const isSameLayer = Math.abs((sy - NODE_H) - ty) < 8
  if (isSameLayer) {
    const loopY = sy + 28
...
function EdgeLayer({ edges, positions }) {
  ...
  return (
    <>
      {edges.map(edge => {
        ...
        const d = elbowPath(p.sx, p.sy, p.tx, p.ty)

        return (
          <g key={k} pointerEvents="none">
            {/* Dark halo for contrast against node backgrounds */}
            <path d={d} fill="none" stroke="#09090b" strokeWidth={4} strokeOpacity={0.8} />
            {/* Colored edge line */}
            <path
              d={d}
              fill="none"
              stroke={color}
              strokeWidth={1.75}
              strokeOpacity={isDegraded ? 0.3 : 0.65}
...
// Port dots — rendered above nodes so connection points are always visible
function PortDots({ edges, positions }) {
  const ports = computePorts(edges, positions)
  ...
  return (
    <>
      {edges.map(edge => {
        ...
        return (
          <g key={k} pointerEvents="none">
            {/* Source dot (bottom of source node) */}
            <circle cx={p.sx} cy={p.sy} r={4} fill={color} fillOpacity={0.9} stroke="#09090b" strokeWidth={1.5} />
            {/* Target dot (top of target node) */}
            <circle cx={p.tx} cy={p.ty} r={4} fill={color} fillOpacity={0.9} stroke="#09090b" strokeWidth={1.5} />
          </g>
        )
      })}
    </>
...
function ServiceNode({ node, pos, onClick, isSelected }) {
...
export default function ServiceMap() {
  ...
  const { positions, svgWidth, svgHeight } = computeLayout(nodes)
      ...
      <div className="relative bg-theme-card border border-theme-border rounded-xl overflow-auto">
        ...
        >
          ...
          {LAYER_ORDER.map(layer => {
            const layerNodes = nodes.filter(n => n.category === layer)
          ...
          {/* Port dots — above nodes so connection points are always visible */}
          <PortDots edges={edges} positions={positions} />
        </svg>
